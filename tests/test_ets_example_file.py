...
class TestETSTextOutput:
    ...
    def test_reset_method_in_output(self) -> None:
        assert "resetInterface" in self.text

    def test_echo_uint8_method_in_output(self) -> None:
        assert "echoUINT8" in self.text

    def test_event_uint8_in_output(self) -> None:
    ...
    def test_vlan_channel_in_output(self) -> None:
...
class TestETSWiresharkService:
    ...
    def test_service_registered(self) -> None:
    ...
    def test_service_major_version(self) -> None:
...
class TestETSWiresharkOutput:
    ...
    def _write(self, tmp_path: Any) -> Any:
    ...
    def test_services_file_header(self, tmp_path: Any) -> None:
        services, _, _ = self._write(tmp_path)
        assert "# This file is automatically generated" in services

    def test_service_entry_in_services_file(self, tmp_path: Any) -> None:
        services, _, _ = self._write(tmp_path)
        assert '"0101","EnhancedTestabilityService"' in services

    def test_reset_method_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","0001","resetInterface"' in methods

    def test_echo_uint8_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","0008","echoUINT8"' in methods

    def test_echo_common_datatypes_0x23_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","0023","echoCommonDatatypes"' in methods

    def test_subscribe_eventgroup_0x32_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","0032","clientServiceSubscribeEventgroup"' in methods

    def test_event_uint8_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","8001","TestEventUINT8"' in methods

    def test_event_multicast_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","800b","TestEventUINT8Multicast"' in methods

    def test_field_getter_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","0025","InterfaceVersion_Getter"' in methods

    def test_field_setter_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","0026","TestFieldUINT8_Setter"' in methods

    def test_field_notifier_in_methods_file(self, tmp_path: Any) -> None:
        _, methods, _ = self._write(tmp_path)
        assert '"0101","8005","InterfaceVersion_Notifier"' in methods

    def test_eg2_all_in_eventgroups_file(self, tmp_path: Any) -> None:
        _, _, eventgroups = self._write(tmp_path)
        assert '"0101","0002","eg2_all"' in eventgroups

    def test_eg5_unreliable_in_eventgroups_file(self, tmp_path: Any) -> None:
        _, _, eventgroups = self._write(tmp_path)
        assert '"0101","0005","eg5_events_and_fields_unreliable"' in eventgroups

    def test_eg6_multicast_in_eventgroups_file(self, tmp_path: Any) -> None:
        _, _, eventgroups = self._write(tmp_path)
