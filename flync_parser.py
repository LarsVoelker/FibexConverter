...
class FlyncParser:
    ...
    def parse_dir(self, conf_factory, directory, verbose=False):
        ...
        self._ft_by_can_id = {}  # (bus_name, can_id) → ft object (for CAN interface lookup)
        self._can_frame_triggerings_by_ecu = {}
    ...
    def _compu_consts_for(sig):
        ...
        if enc is None:
            return []
        if isinstance(enc, TextTable):
            return [(e.label, str(e.from_value), str(e.to_value)) for e in enc.entries]
        print(f"WARNING: Signal {sig.name!r} uses value_encoding type {type(enc).__name__}, which has no FIBEX COMPU-METHOD equivalent; skipping")
        return []

    _FLYNC_TO_FIBEX_USAGE_MAPPING = {
        "application": "APPLICATION",
        "bap": "BAP",
        "diag_request": "DIAG-REQUEST",
        "diag_response": "DIAG-RESPONSE",
        "diag_state": "DIAG-STATE",
        "network_management": "NM",
        "other": "OTHER",
        "service": "SERVICE",
        "tpl": "TPL",
    ...
    def _parse_channels(self, conf_factory, flync_model, verbose=False):
        ...
        for flync_pdu in channels.pdus or []:
            ...
            for si in flync_pdu.signals:
                ...
                base_sig = conf_factory.create_signal(
                    ...
                    0,
                    basetype,
                    sig.data_type.natural_bit_width(),
                )
                si_id = sig.name + "_si"
            ...
            for flync_frame in bus.frames:
                ...
                if base_frame is None:
                    ...
                    base_frame = conf_factory.create_frame(
                        ...
                        flync_frame.name,
                        flync_frame.length,
                        self._map_frame_usage(flync_frame),
                        pdu_insts,
                    )
                    base_frame_by_name[flync_frame.name] = base_frame
                ft = conf_factory.create_frame_triggering_can(
                    f"{bus.name}-{flync_frame.name}-0x{flync_frame.can_id:x}_ft",
                    base_frame,
                    flync_frame.can_id,
                    is_extended_id=flync_frame.id_format == "extended_29bit",
                    is_can_fd=flync_frame.type == "can_fd",
                )
                # Index by (bus, CAN ID) so _parse_ecus can look up FTs from
                # can_interfaces (CANFrameRef references frames by CAN ID).
                self._ft_by_can_id[(bus.name, flync_frame.can_id)] = ft
        ...
        self._base_pdu_by_name = base_pdu_by_name

        # Build PDU name → header_id map.
        # Container PDUs have an explicit pdu_id; standard/multiplexed PDUs get a synthetic one.
        for container_pdu in channels.ethernet_pdu_containers or []:
            self._pdu_header_id_by_name[container_pdu.name] = container_pdu.pdu_id
        for pdu_name in base_pdu_by_name:
            if pdu_name not in self._pdu_header_id_by_name:
    ...
    def _parse_multiplex_pdu(self, flync_pdu, conf_factory, base_pdu_by_name):
        ...
        for k, v in meta.get("mux_pdu_refs", {}).items():
            try:
                ...
                print(f"WARNING: Skipping non-numeric mux_pdu_refs key {k!r} in multiplex PDU {flync_pdu.name!r}")
    ...
    def _parse_services(self, conf_factory, flync_model, verbose=False):
        ...
        if flync_model.communication.someip_config is None:
            return
        for svc in flync_model.communication.someip_config.services:
            if verbose:
    ...
    def _resolve_pdu(self, conf_factory, pdu_ref):
        ...
        _parse_channels.  Container PDUs are only materialised here when a
        PDUSender/PDUReceiver deployment actually references them, so workspaces
        without such deployments keep exactly the same output as the FIBEX path.
        """
        base_pdu = self._base_pdu_by_name.get(pdu_ref)
        if base_pdu is not None:
            return base_pdu
            ...
            for socket in socket_container.sockets or []:
                ...
                for dep_union in socket.deployments or []:
                    ...
                    if isinstance(dep, SOMEIPServiceProvider):
                        ...
                        if svc_id is None:
                            print(f"WARNING: Skipping SOMEIPServiceProvider on socket {socket.name!r}: non-coercible service ref {dep.service!r}")
                            continue
                        ...
                        svc_id = self._deployment_service_id(dep)
                        if svc_id is None:
                            print(f"WARNING: Skipping SOMEIPServiceConsumer on socket {socket.name!r}: non-coercible service ref {dep.service!r}")
                            continue
                        svc_obj = self._services_by_id.get((svc_id, dep.major_version))
                        if svc_obj is not None:
                            try:
                    ...
                    elif isinstance(dep, PDUSender):
                        base_pdu = self._resolve_pdu(conf_factory, dep.pdu_ref)
                        header_id = self._pdu_header_id_by_name.get(dep.pdu_ref)
                        if base_pdu is not None and header_id is not None:
                            eth_pdu_inst = conf_factory.create_ethernet_pdu_instance(dep.pdu_ref, header_id)
                            eth_pdu_inst.add_pdu(base_pdu)
                            outgoing_pdus.append(eth_pdu_inst)
                        ...
                        if base_pdu is not None and header_id is not None:
                            eth_pdu_inst = conf_factory.create_ethernet_pdu_instance(dep.pdu_ref, header_id)
                            eth_pdu_inst.add_pdu(base_pdu)
                            incoming_pdus.append(eth_pdu_inst)
    ...
    def _parse_switches(self, conf_factory, flync_model, verbose=False):
        ...
        for ecu in flync_model.ecus:
            ...
            for sw in ecu.switches:
                ...
                for flync_sp in sw.ports:
                    ...
                    sp_list.append(sp)
                    swport_map[(ecu.name, sw.name, flync_sp.name)] = sp
        ...
        for ecu in flync_model.ecus:
            ...
            for sw in ecu.switches:
                for flync_sp in sw.ports:
                    base_sp = swport_map.get((ecu.name, sw.name, flync_sp.name))
                    if base_sp is None:
                        continue
                    ...
                    if comp is None:
                        ...
                        peer_base_sp = swport_map.get((ecu.name, comp.get_switch().name, comp.name))
                        ...
                        if peer_internal is not None and peer_internal.type == "switch_port":
                            ...
                            if peer_ecu_obj is None:
                                continue
                            peer_base_sp = swport_map.get((peer_ecu_obj.name, peer_internal.get_switch().name, peer_internal.name))
                            if peer_base_sp is not None:
                                if base_sp.connected_to_port() is None:
                                    base_sp.set_connected_port(peer_base_sp)
                                if peer_base_sp.connected_to_port() is None:
