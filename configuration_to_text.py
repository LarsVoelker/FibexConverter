...
class SimpleConfigurationFactory(BaseConfigurationFactory):
    ...
    def add_service(self, serviceid, majorver, minorver, service):
        ...
        if sid in self.__services__:
            logger.error(
                "Service (SID: 0x%04x, Major-Ver: %d) already exists with a different Minor Version (not %d)! " "Not overriding it!",
                serviceid,
                majorver,
                minorver,
            )
...
class SOMEIPServiceEventgroup(SOMEIPBaseServiceEventgroup):
    ...
    def str(self, indent: int) -> str:
        ...
        if len(self.__fieldids__) > 0:
            ret += (2 + indent) * " "
            if None in self.__fieldids__:
                logger.error("Field is referenced by Eventgroup %s but does not have Notifier!", self.__name__)
