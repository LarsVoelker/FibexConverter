...
from flync.model.flync_4_ecu.controller import EthernetInterfaceConfig, VirtualControllerInterface
...
class SimpleConfigurationFactory(BaseConfigurationFactory):
            ...
            for base_switch in base_ecu.switches():
                for base_port in base_switch.ports():
                    ...
                    if peer_port is not None:
                        ...
                        own_port_id = base_port.portid(gen_name=g_gen_portid)
                        peer_port_id = peer_port.portid(gen_name=g_gen_portid)
                        pair = frozenset({(ecu_name, own_port_id), (peer_ecu_name, peer_port_id)})
                        if pair in seen_pairs:
                            continue
                        seen_pairs.add(pair)

                        if peer_ecu_name == ecu_name:
                            # Intra-ECU switch-to-switch: model as SwitchPortToSwitchPort
                            # (no ECUPort hops, no system-topology entry).
                            intra_ecu_swsw_pairs.append(
                                (
                                    ecu_name,
                                    own_port_id,
                            ...
                            # carry an MII config with complementary mac/phy modes. Pick
                            # mac for the first port we see and phy for its peer.
                            intra_ecu_swsw_mii_mode[(ecu_name, own_port_id)] = "mac"
                            intra_ecu_swsw_mii_mode[(ecu_name, peer_port_id)] = "phy"
                            continue

                        # Cross-ECU switch-to-switch: synthesize ECUPorts on each side.
                        # Assign roles via heuristics: alphabetically smaller ECU name is master
                        if ecu_name <= peer_ecu_name:
                            role_map[(ecu_name, own_port_id)] = "master"
                            role_map[(peer_ecu_name, peer_port_id)] = "slave"
                        else:
                            role_map[(ecu_name, own_port_id)] = "slave"
                            role_map[(peer_ecu_name, peer_port_id)] = "master"
                        ext_connections.append((ecu_name, own_port_id, peer_ecu_name, peer_port_id))
        ...
        for ecu_name, base_ecu in sorted(self.base_ecus().items()):
            ...
            for base_switch in base_ecu.switches():
                ...
                for i, base_port in enumerate(base_switch.ports()):
                    ...
                    flync_swport_name_map[(ecu_name, sw_name, port_id)] = sp_name
                    ...
                    # complementary RGMII mac/phy (assigned in Step 1). External
                    # switch ports (going through ECUPorts) have no MII config.
                    swsw_mii_mode = intra_ecu_swsw_mii_mode.get((ecu_name, port_id))
                    if is_cpu_port:
                        sp_mii_config = RGMII(type="rgmii", mode="mac", speed=1000)
                    elif swsw_mii_mode is not None:
                        sp_mii_config = RGMII(type="rgmii", mode=swsw_mii_mode, speed=1000)
                    else:
                        sp_mii_config = None
        ...
        for ecu_name, base_ecu in sorted(self.__base_ecus.items()):
            ...
            for fibex_ctrl in sorted(fibex_controllers, key=lambda c: c.name()):
                ...
                if fibex_ifaces:
                            ...
                            if ip_obj.version == 4:
                                ...
                                if ip_addr.is_multicast:
                                    ...
                                    try:
                                        ...
                                        print(f"WARNING: Invalid IPv4 netmask {netmask_str!r} for {ip_str}: {e}; falling back to 255.255.255.0")
                                ...
                                if ip_addr.is_multicast:
                                    ...
                                    try:
                                        ...
                                        print(f"WARNING: Invalid IPv6 prefix length {prefix_len!r} for {ip_str}: {e}; falling back to 128")
                                        ipv6prefix = 128
                                    addresses.append(
                                        IPv6AddressEndpoint(
                                            address=ip_addr,
                    ...
                    if virt_ifaces:
                        iface_name = f"{self._safe_name(ecu_name)}_{self._safe_name(fibex_ctrl.name())}_iface"
                ...
                can_interfaces = [
                    CANInterface(
                        ...
                        bus_ref=bus_ref,
                        sender_frames=[CANFrameRef(bus_ref=bus_ref, frame_ref=cid) for cid in sorted(fdata["sender"])],
                        receiver_frames=[CANFrameRef(bus_ref=bus_ref, frame_ref=cid) for cid in sorted(fdata["receiver"])],
                    )
            ...
            if has_switches:
                ...
                # (collected in Step 1 as intra_ecu_swsw_pairs).
                for ecn, port1_id, sw1_name, port2_id, sw2_name in intra_ecu_swsw_pairs:
                    if ecn != ecu_name:
                        continue
                    sp1_name = flync_swport_name_map.get((ecu_name, sw1_name, port1_id))
    ...
    _FIBEX_TO_FLYNC_USAGE_MAPPING = {
        "APPLICATION": "application",
        "BAP": "bap",
        "DIAG-REQUEST": "diag_request",
        "DIAG-RESPONSE": "diag_response",
        "DIAG-STATE": "diag_state",
        "NM": "network_management",
        "OTHER": "other",
        "SERVICE": "service",
        "TPL": "tpl",
    ...
    def _create_flync_channels(self):
        ...
        # Set of frame IDs that have at least one CAN FT (used by the Ethernet
        # container fallback below to skip frames already emitted as CAN frames).
            ...
            try:
                ...
                for name, start, end in base_sig.compu_consts() or []:
                    try:
                        ...
                        continue
                    if lo == hi:
                        if lo in single_entries:
                            print(
                                f"WARNING: Overwriting value encoding for value {lo}: "
                                f"{single_entries[lo]} -> {name}. "
                                f"Signal: {base_sig.name()}!"
                            )
                        single_entries[lo] = name
                    else:
                        range_entries.append((lo, hi, name))

                if range_entries:
                    entries: list[TextEntry] = []
                    for lo, hi, name in range_entries:
                        te = TextEntry(from_value=lo, to_value=hi, label=name)
                        entries.append(te)
                    for lo, name in single_entries.items():
                        te = TextEntry(from_value=lo, to_value=lo, label=name)
                        entries.append(te)
                    if entries:
                        value_encoding = TextTable(type="text_table", entries=entries)
                elif single_entries:
                    entries_tt: list[TextEntry] = []
                    for v, label in single_entries.items():
                        entries_tt.append(TextEntry(from_value=v, to_value=v, label=label))
                    if entries_tt:
                        value_encoding = TextTable(type="text_table", entries=entries_tt)
                ...
                print(f"          value_encoding={value_encoding})")
        ...
        # We emit one FLYNC frame per FIBEX frame-triggering. FLYNC frames no
            ...
            try:
                ...
                if ft.is_can_fd():
                    ...
                    flync_frame = CANFDFrame(
                        ...
                        frame_usage=self._flync_frame_type(base_frame),
                    )
    ...
    def create_flync_model(self):
        ...
        general_config = FLYNCCommunicationConfig(
